...
export class BookingsService {
    ...
    async myAsPassenger(userId: string, q: BookingsQueryDto) {
        ...
        const [items, total] = await this.prisma.$transaction([
            this.prisma.booking.findMany({
                ...
                take: pageSize,
                include: {
                    trip: { include: { fromCity: true, toCity: true, driver: { select: { id: true, phone: true } } } },
                    request: true,
                    payments: true,
                },
            }),
    ...
    async getByIdForUser(bookingId: string, userId: string) {
        ...
        if (!isPassenger && !isDriver) throw new ForbiddenException('No access to this booking');
     ...
     */
    async cancelAsPassenger(bookingId: string, userId: string, dto: CancelBookingDto) {
     ...
     * - статус confirmed
     */
    async cancelAsDriver(bookingId: string, driverId: string, dto: CancelBookingDto) {
        return this.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
            ...
            await tx.tripRequest.update({
                where: { id: booking.requestId },
                data: { status: 'canceled', respondedAt: new Date(), rejectionReason: dto.reason ?? null },
            });
